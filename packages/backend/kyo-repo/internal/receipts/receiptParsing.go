...
func (rec ExcelizeParser) parseWorkbook(filePath string) (*ParsedReceipt, error) {
	...
	for sheetIndex := 0; sheetIndex < workbook.SheetCount; sheetIndex++ {
		...
		switch sheetIndex {
		case 0:
			array, err := rec.parseExpenses(&rows)
			if err != nil {
				return nil, err
			}
			expensesArray = array
		...
		case 2, 4:
			array, err := rec.parseExtraCharges(&rows)
			if err != nil {
				return nil, err
			}
...
func (rec ExcelizeParser) parseExpenses(rows *[][]string) ([]model.Expenses, error) {
...
func (rec ExcelizeParser) parseDebts(rows *[][]string) ([]model.Debts, error) {
...
func (rec ExcelizeParser) parseExtraCharges(rows *[][]string) ([]model.ExtraCharges, error) {
